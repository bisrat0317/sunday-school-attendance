...
async function exportMasterAttendanceMatrixToExcel() {
  ...
  const isAmharic = currentLang === 'am';
  showToast('Generating Master Attendance Matrix Excel...', 'info');

  try {
    const data = await api('/api/reports/master-attendance-matrix');
    if (!data || !data.students || data.students.length === 0) {
      showToast('No active students found to generate matrix', 'warning');
      return;
    }

    const { students, sessions, attendance } = data;

    // Create Attendance Lookup Map: session_id + "_" + student_id -> record
    const attMap = {};
    ...
    const defaultCategories = ['Child', 'Teens', 'Youth', 'Adult'];
    const activeCategories = Array.from(new Set([...defaultCategories, ...(data.categories || [])]));

    activeCategories.forEach(cat => {
      const catStudents = students.filter(s => s.category === cat);
      const catSessions = sessions.filter(s => s.category === cat || s.category === 'All');

      if (catStudents.length === 0) return; // Skip empty categories

      const catRows = catStudents.map((s, idx) => {
        const row = {
        ...
        catSessions.forEach(sess => {
          ...
          const rec = attMap[`${sess.id}_${s.id}`];
      ...
      // Sheet Tab Name
      let sheetName = cat;
      if (cat === 'Child') sheetName = isAmharic ? 'ህፃናት' : 'Child';
      if (cat === 'Teens') sheetName = isAmharic ? 'አዳጊዎች' : 'Teens';
      if (cat === 'Youth') sheetName = isAmharic ? 'ወጣቶች' : 'Youth';
      if (cat === 'Adult') sheetName = isAmharic ? 'አዋቂዎች' : 'Adult';

      const worksheet = XLSX.utils.json_to_sheet(catRows);
